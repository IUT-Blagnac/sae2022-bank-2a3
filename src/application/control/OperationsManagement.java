...
public class OperationsManagement {
	...
	public Operation enregistrerVirement() {
		...
		OperationEditorPaneVirement oep = new OperationEditorPaneVirement(this.primaryStage, this.dbs);
		Operation op = oep.doOperationEditorDialog(this.compteConcerne, CategorieOperation.VIREMENT);
		if (op != null) {
			try {
				AccessOperation ao = new AccessOperation();
				AccessCompteCourant acc = new AccessCompteCourant();
				int compteVise = oep.getCompteVirement();
				CompteCourant compte;
				compte = acc.getCompteCourant(compteVise);

				ao.insertVirement(this.compteConcerne.idNumCompte, op.montant, op.idTypeOp, compte.idNumCompte  );
