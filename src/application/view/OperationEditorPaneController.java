...
public class OperationEditorPaneController implements Initializable {
	...
	public Operation displayDialog(CompteCourant cpte, CategorieOperation mode) {
		...
		case CREDIT:
			...
			for (String tyOpC : ConstantesIHM.OPERATIONS_CREDIT_GUICHET) {
				listC.add(tyOpC);
			}

			this.cbTypeOpe.setItems(listC);
			this.cbTypeOpe.getSelectionModel().select(0);
		 break;
		case VIREMENT:
			...
			for (String tyOpV : ConstantesIHM.OPERATIONS_VIREMENT_GUICHET) {
				listV.add(tyOpV);
			}

			this.cbTypeOpe.setItems(listV);
			this.cbTypeOpe.getSelectionModel().select(0);
		 break;
		}
	...
	@FXML
	public int getCompteV() {
		return Integer.parseInt(txtCompteV.getText());
	}
	@FXML
